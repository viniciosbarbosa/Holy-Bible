...
import { CheckCircle2, Bookmark, Download, AlertCircle, Settings } from "lucide-react";
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        ...
        <div className="flex gap-1.5">
          {currentStatus === "missing" && <AlertCircle size={14} className="text-red-500/80" />}
          {currentStatus === "acquired" && <Bookmark size={14} className="text-green-500/80" />}
          {currentStatus === "downloaded" && <Download size={14} className="text-blue-500/80" />}
          {isRead && <CheckCircle2 size={16} className="text-bible-gold drop-shadow-[0_0_8px_rgba(201,168,76,0.4)]" />}
        </div>
      </div>

      {/* Content */}
      <div className="relative z-10 mb-6 flex-1">
        <h4 className={`font-serif text-lg font-bold mb-1 transition-colors duration-300 ${isRead ? "line-through text-bible-muted" : "text-bible-text group-hover:text-bible-gold"}`}>
          {book.name}
        </h4>
        <p className="text-[11px] text-bible-muted italic line-clamp-2 leading-relaxed font-serif">
          {book.sub || "No description available."}
        </p>
