...
import { ArrowLeft, ChevronLeft, ChevronRight, Type, Share2, Bookmark } from "lucide-react";
...
export default function BibleReader() {
  ...
  const { data, isLoading, isError } = useBibleChapter(translation, bookId || "", chapterNumber);
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center py-48 space-y-6">
      ...
      <p className="font-cinzel text-sm text-bible-gold animate-pulse tracking-[0.3em] uppercase">Abrindo Manuscritos...</p>
    </div>
  );

  if (isError || !data) return (
    <div className="flex flex-col items-center justify-center py-48 space-y-6 text-center">
      <p className="text-red-500 font-serif text-xl italic">Os manuscritos deste capítulo parecem inacessíveis.</p>
      <button onClick={() => navigate("/common")} className="px-8 py-3 rounded-2xl bg-bible-gold text-white font-cinzel text-xs uppercase tracking-widest shadow-xl">
        Retornar ao Cânone
      </button>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto px-4 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <header className="relative mb-16 p-8 md:p-12 rounded-[3rem] bg-bible-card/40 backdrop-blur-xl border border-bible-border shadow-2xl overflow-hidden text-center">
        ...
        <div className="absolute top-8 right-8 flex items-center gap-4 bg-bible-card/60 p-2 px-4 rounded-2xl border border-bible-border">
          <button onClick={() => setFontSize(fontSize - 2)} className="text-bible-muted hover:text-bible-gold transition-colors">
            <Type size={14} />
          </button>
          <span className="font-cinzel text-[10px] text-bible-gold w-8">{fontSize}px</span>
          <button onClick={() => setFontSize(fontSize + 2)} className="text-bible-muted hover:text-bible-gold transition-colors">
            <Type size={18} />
          </button>
        </div>
      ...
      >
        {data.chapter.content.map((verse, idx) => {
          const isFavorited = favoriteVerses.some(v => 
            v.bookAbbrev === bookId && 
          ...
          return (
            <p key={idx} className="relative pl-12 group">
              ...
              <button 
                onClick={() => {
                  if (isFavorited) {
                    const fav = favoriteVerses.find(v => v.bookAbbrev === bookId && v.chapter === chapterNumber && v.verse === verse.number);
      ...
      <footer className="flex flex-col gap-12 items-center mb-32">
        <div className="flex items-center gap-6">
          ...
          <div className="flex gap-4">
            <button 
              onClick={() => alert(`Marcador salvo no capítulo ${data.chapter.number}`)}
